...
import pandas as pd
engine = create_engine('sqlite:///sqlite_db_pythonsqlite.db')
df = pd.read_sql_query('SELECT M1.surname, M1.firstname, M2.surname AS Recommender_surname, M2.firstname AS Recommender_firstname\
                       FROM Members AS M1 \
...
df = pd.read_sql_query('SELECT name, \
                       memid, \
                       SUM(slots) AS Total_member_slots \
                       FROM Bookings \
                       LEFT JOIN Facilities \
                       USING (facid) \
                       WHERE memid <> 0 \
                       GROUP BY memid, name', engine)
...
df = pd.read_sql_query('SELECT name, \
                       ...
                       SUM(CASE WHEN memid <> 0 THEN slots END) AS Total_member_slots \
                       FROM Bookings \
                       LEFT JOIN Facilities \
                       USING (facid) \
